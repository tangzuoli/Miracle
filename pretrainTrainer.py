...
class PretrainTrainer(object):
    def __init__(self, model, config, train_dataloader, valid_dataloader, test_dataloader):
        ...
        self.losses_types = ['all', 'recommendation', 'balance', 'aspect contrastive', 'disen', 'contras']
        ...
        if self.config['load_model_path'] == "" or self.config['stage'] != 'train':
            ...
            self.saved_path = "./saved_model/{}-{}-{}-{}-{}-{}-{}-{}/".format(self.config['aspect_k'],
                                                                              self.config['aspects'],
                                                                              self.config['stage'],
                                                                              self.config['dataset'],
                                                                              local_time.tm_mday,
    ...
    def train(self):
        ...
        if self.config['stage'] == 'pretrain':
            ...
            return self.saved_path + f"pretrain-{self.config['epoch']-1}.pkl"
