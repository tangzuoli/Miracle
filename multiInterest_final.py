...
from sub_module.sequential_recommender import SequentialRecommender
...
class MoERouteLayer(nn.Module):
    def __init__(self, config):
        super(MoERouteLayer, self).__init__()
    ...
    def forward_item(self, item_emb):
        ...
        gates = self.generate_gates(item_emb)  # batch_size * neg_count/1 * pool
        return item_emb.unsqueeze(-2), F.softmax(gates / self.tau, dim=-1)


class MultiInterest(SequentialRecommender):
    def __init__(self, config):
        super(MultiInterest, self).__init__(config)
        ...
        self.mmoe_layer = MoERouteLayer(config)
        self.pad_mode = config['pad_mode']
        self.neg_count = config['neg_count']
        self.item_embs = None
        self.balance_alpha = config['balance_alpha']
        self.infinity_alpha = config['infinity_alpha']
        self.aspect_k = config['aspect_k']
        self.aspects = config['aspects']
        self.time_aware_attn = TimeAwareAttention(config)
    ...
    def seq_encode(self, item_seq, item_emb, item_seq_len):
        """
        item_seq: bs * lengths
        item_emb: bs * lengths * hidden_size

        return:  bs * lengths * hidden_size
        """
    ...
    def calculate_pretrain_loss(self, interaction):
        ...
        return [
            ...
            loss, balance_loss, aspect_contrastive_loss, mi_cs_loss]
    ...
    def calculate_downstream_loss(self, interaction):
        ...
        total_embs, item_gates = self.mmoe_layer.forward_item(total_embs)  # n_items * aspects * hidden_size
        total_embs = total_embs.squeeze(1)
        ...
        return [loss + self.balance_alpha * balance_loss + self.aspect_alpha * aspect_contrastive_loss,
                loss, balance_loss, aspect_contrastive_loss]
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.stage == 'trans':
            total_embs = total_embs + self.item_embedding.weight
        total_embs, item_gates = self.mmoe_layer.forward_item(total_embs)
    ...
    def cal_aspects_contrastive_loss(self):
        embs = self.mmoe_layer.aspect_embs.weight
    ...
    def cal_mI_contrastive_loss(self, interaction, interests, aspect_mask, gates):
        ...
        interests_aug, gates_aug, aspect_mask_aug = self.mmoe_layer.forward_sequence(seq_output_aug,
                                                                                     item_seq,
    ...
    def seq_aug(self, item_seq, item_seq_len):
        ...
        item_seq_aug = torch.masked_fill(item_seq, mask, self.mask_idx)
